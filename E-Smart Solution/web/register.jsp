<%-- 
    Document   : register
    Created on : Mar 10, 2016, 1:55:57 PM
    Author     : dell-lap
--%>
...
<%@page import="java.sql.*,mysqlC.conn" %>  
    <jsp:useBean id="form" class="org.timtroy.bean.MyForm" scope="request">  
    ...
    <%  
            ...
            if (form.process()) {  
                ...
                ResultSet rst=stm.executeQuery("select MAX(user_id) as user_id FROM user");
                if(rst.next())
                {
                id=rst.getInt(1);
                }
                id++;
               stm.executeUpdate("insert into user values('"+id+"','"+u+"','"+p+"','"+n+"','"+a+"','"+c+"','"+e+"','"+ar+"','"+String.valueOf(code)+"','0')");
                hs.setAttribute("email",e);
                hs.setAttribute("code",code);
               response.sendRedirect("MailPage123");
...
<%!  
        ...
        public void jspInit() {  
            ...
            errorMap.put(MyForm.ERR_ADD_BLANK, "Please enter your address");  
            errorMap.put(MyForm.ERR_CONTACT_BLANK, "Please enter your mobile no.");  
            errorMap.put(MyForm.ERR_CONTACT_LENGTH, "Contact should be 10 digit");  
            errorMap.put(MyForm.ERR_CONTACT_INVALID, "Invalid contact no.");

           errorMap.put(MyForm.ERR_EMAIL_BLANK, "Please enter your email id");  
            errorMap.put(MyForm.ERR_EMAIL_INVALID, "Please enter valid email id");  
            errorMap.put(MyForm.ERR_USER_BLANK, "Please enter your username");  
            errorMap.put(MyForm.ERR_USER_EXISTS, "Username already exists");  
            errorMap.put(MyForm.ERR_PASS_BLANK, "Please enter your password"); 
            errorMap.put(MyForm.ERR_PASS_LENGTH, "Length should be 5 or more");  
            errorMap.put(MyForm.ERR_PASS_MATCH, "Password does not match");  
