<%-- 
    Document   : myAssignment
    Created on : Mar 11, 2016, 1:46:39 AM
    Author     : dell-lap
--%>
...
String u=(String)hs.getAttribute("eUser");   
if(u==null)
   {
...
int id=0;
ResultSet rst1,rst2;
        rst1=stm.executeQuery("select * from engineer where username='"+u+"'");
if(rst1.next())
{
id=rst1.getInt("emp_id");
}
        rst1.close();
 rst2=stm.executeQuery("select * from complaintsassigned where EnggId='"+id+"' and EnggStatus='0'");
 %>
...
%>
         <tr style="background-color: yellowgreen">
             <td><%=rst2.getInt("complaintId")%></td>
             <td><%=rst2.getString("address") %></td>
             <td><%=rst2.getString("area") %></td>
             <td><%=rst2.getString("c_time") %></td>
             <td><%=rst2.getString("f_time") %></td>
             <td><%=rst2.getString("EnggStatus") %></td>
             <td><%=rst2.getString("prob_type") %></td>
             <td><%=rst2.getString("prob_desc") %></td>
             <td><a href="completed.jsp?par=<%=rst2.getInt("complaintId")%>"><input type="submit" value="Completed" /></a></td>
             <td><a href="solution.jsp?par=<%=rst2.getInt("complaintId")%>"><input type="submit" value="Solve" color="brown"/></a></td>
         </tr>
         <%
}         
%>
